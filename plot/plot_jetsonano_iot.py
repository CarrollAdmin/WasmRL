...
# ax.set_xlabel("Domains", fontsize=24)
ax.set_ylabel("Relative execution time\n(O0=1)", fontsize=35)
plt.legend(loc='upper center', ncol=3, fontsize=30, bbox_to_anchor=(0.5, 1.14), frameon=False, handletextpad=0.4)
ax.set_ylim([0, 1])
ax.yaxis.set_major_locator(MultipleLocator(0.2)) # 设置y轴坐标显示倍数
ax.yaxis.set_tick_params(labelsize=25)
ax.set_xticks(xticks, datasets, rotation=30, fontsize=35, ha='right')
# plt.tick_params(labelsize=35)
plt.show()
plt.savefig("overall_jetson_iot.pdf", bbox_inches='tight')
plt.close()