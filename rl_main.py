...
def main(applications, is_prior):
    
    if applications == "polybenchc":
        bms = [
            "2mm",
            "3mm",
            "adi",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run PPO optimization for WASM applications.")
    parser.add_argument("--applications", type=str, required=True, 
                        choices=["polybenchc", "cbench", "singleglass"],
                        help="Specify the applications to use.")
    parser.add_argument("--is_prior", action="store_true", 
                        help="Specify whether to use prior knowledge during training.")
    
    args = parser.parse_args()

    main(applications=args.applications, is_prior=args.is_prior)
